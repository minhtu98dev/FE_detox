...
//   };
  ...
  const closePopupDelete = () => {
    ...
    const fetchDataSearch = async () => {
      try {
          ...
          setTimeout(async () => {
            const fetchedOrders = await fetch(`${backendUrl}/api/get/contact/name/${search}`);
            const json = await fetchedOrders.json();
            console.log("fetchedOrders: ", json);
  ...
  return (
            ...
            {currentcontacts.map((contact, index) => (
              <tr key={contact.id}>
                ...
                <td>
                  ...
                  <button className='bt-delete'onClick={() => openPopupDelete({ uuid: contact.uuid })}><AiOutlineDelete></AiOutlineDelete></button>
                </td>
              </tr>
            ))}
    ...
    <path d="M16 19V5l-11 7z" transform="rotate(120 12 12)"/>
              ...
              {Array.from({ length: Math.ceil(contacts.length / ordersPerPage) }, (_, i) => (
                  <button
                      key={i}
              ...
              <button className='btnext' onClick={nextPage} disabled={currentPage === Math.ceil(contacts.length / ordersPerPage)}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
    <path d="M0 0h24v24H0z" fill="none"/>
