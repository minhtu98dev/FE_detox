...
const Order = () => {
  ...
  const fetchDataSearchName = async () => {
    try {
        // Thêm độ trễ 1 giây (1000 miliseconds) trước khi gọi API
        setTimeout(async () => {
          const fetchedOrders = await fetch(`${backendUrl}/api/get/order/name/${search}`);
          const json = await fetchedOrders.json();
          console.log("fetchedOrders: ", json);
          setOrders(json);
        }, 100);
    ...
    const fetchDataSearchOrderId = async () => {
      try {
          // Thêm độ trễ 1 giây (1000 miliseconds) trước khi gọi API
          setTimeout(async () => {
            const fetchedOrders = await fetch(`${backendUrl}/api/get/order/code/${search}`);
            const json = await fetchedOrders.json();
            console.log("fetchedOrders: ", json);
            setOrders(json);
          }, 100);
      ...
      const fetchDataSearchSdt = async () => {
        try {
            // Thêm độ trễ 1 giây (1000 miliseconds) trước khi gọi API
            setTimeout(async () => {
              const fetchedOrders = await fetch(`${backendUrl}/api/get/order/phone/${search}`);
              const json = await fetchedOrders.json();
              console.log("fetchedOrders: ", json);
              setOrders(json);
            }, 100);
      ...
      <table>
        ...
        <tbody>
          {currentOrders.map((order, index) => (
            <tr key={order.id}>
              <td onClick={() => handleCustomerClick(order.uuid)}>{index + 1}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{order.id_order}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{order.name}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{order.phone}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{convert_time(order._create_time)}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{order.billTotal}</td>
              <td onClick={() => handleCustomerClick(order.uuid)}>{order.payment}</td>
              <td onClick={() => handleCustomerClick(order.uuid)} className={
        ...
        <div className="popup">
            ...
            <select onChange={handleEditStatus} >
