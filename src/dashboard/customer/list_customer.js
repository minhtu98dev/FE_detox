...
const ListCustomer = ({ list_customers, onDelete }) => {
  ...
  const getUserNameByUUID = (uuid) => {
    const customerWithUUID = customers.find(
    ...
    try {
      ...
      setTimeout(async () => {
        const fetchedOrders = await fetch(
          `${backendUrl}/api/get/customer/name/${search}`
        );
        const json = await fetchedOrders.json();
        console.log("fetchedOrders: ", json);
        setCustomer(json.data);
  ...
  return (
    <div className="admin-order">
      ...
      <h3 className="mt-5 mb-4">KHÁCH HÀNG</h3>

      <div className="top">
        <div className="top-left mb-2">
          <div className="show">
            Khách hàng/trang
      ...
      <table>
        ...
        <tbody>
          {currentOrders.map((customer, index) => (
            <tr key={customer.id}>
              <td onClick={() => handleCustomerClick(customer.email)}>
                {index + 1}
              </td>
              <td onClick={() => handleCustomerClick(customer.email)}>
                {customer.name}
              ...
              <td>
                ...
                <button
                  className="bt-delete"
                  onClick={() => openPopupDelete({ uuid: customer.uuid })}
                >
      ...
      <div>
        <ul className="pagination">
          ...
          {Array.from(
            { length: Math.ceil(customers.length / ordersPerPage) },
            (_, i) => (
              <button
                key={i}
